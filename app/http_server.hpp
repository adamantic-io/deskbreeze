...
INCTXT_EXTERN(index_css);

using json = nlohmann::json;

inline void init_database(sqlite3* db) {
    // Create pets table
    sqlite3_exec(db, R"(
    ...
    sqlite3_exec(db, R"(
        ...
        ('Buddy', 'Dog', 'Golden Retriever', 2, 800.00, 'Friendly and energetic golden retriever, great with kids', '/images/golden-retriever.jpg', 1),
        ('Luna', 'Cat', 'Persian', 1, 600.00, 'Beautiful Persian cat with long silky fur', '/images/persian-cat.jpg', 1),
        ('Charlie', 'Dog', 'Labrador', 3, 750.00, 'Well-trained Labrador, perfect family dog', '/images/labrador.jpg', 1),
        ('Whiskers', 'Cat', 'Siamese', 2, 550.00, 'Elegant Siamese cat with striking blue eyes', '/images/siamese-cat.jpg', 1),
        ('Tweety', 'Bird', 'Canary', 1, 150.00, 'Beautiful singing canary with bright yellow feathers', '/images/canary.jpg', 1),
        ('Nemo', 'Fish', 'Goldfish', 1, 25.00, 'Healthy goldfish perfect for beginners', '/images/goldfish.jpg', 1),
        ('Fluffy', 'Small Animal', 'Rabbit', 1, 120.00, 'Adorable rabbit with soft white fur', '/images/rabbit.jpg', 1),
        ('Max', 'Dog', 'German Shepherd', 4, 900.00, 'Intelligent and protective German Shepherd', '/images/german-shepherd.jpg', 1),
        ('Mittens', 'Cat', 'Maine Coon', 2, 700.00, 'Large and gentle Maine Coon cat', '/images/maine-coon.jpg', 1),
        ('Polly', 'Bird', 'Parrot', 3, 400.00, 'Colorful parrot that can learn to speak', '/images/parrot.jpg', 1);
    )", nullptr, nullptr, nullptr);
...
inline void start_server() {
    ...
    sqlite3* db;
    sqlite3_open("petstore.db", &db);
    init_database(db);

    // Get all pets with optional filtering
    svr.Get("/api/pets", [db](const Request& req, Response& res) {
        std::string query = R"(
    ...
    svr.Get("/api/pets/(\\d+)", [db](const Request& req, Response& res) {
    ...
    // Get all categories
    svr.Get("/api/categories", [db](const Request& req, Response& res) {
        sqlite3_stmt* stmt;
    ...
    // Create new order
    svr.Post("/api/orders", [db](const Request& req, Response& res) {
    ...
    svr.Get("/api/orders", [db](const Request& req, Response& res) {
        sqlite3_stmt* stmt;
    ...
    // Serve CSS files
    svr.Get("/index.css", [&](const Request&, Response& res) {
        // For now, return empty CSS since styles are inline in React
        res.set_content(std::string(g_index_css_data), "text/css");
    });

    // Handle React Router routes by serving index.html
    svr.Get(R"(/cart.*)", [&](const Request&, Response& res) {
        res.set_content(std::string(g_index_html_data), "text/html");
    });

    svr.Get(R"(/orders.*)", [&](const Request&, Response& res) {
        res.set_content(std::string(g_index_html_data), "text/html");
    });
